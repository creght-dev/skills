 ...
 * skills ship with the profile instead of having to be copied into a project's
 * `.agents/skills` directory.
 * @module creght-skills
 */
...
const PROVIDER_NAME = 'creght'
const SKILLS_ROOT = fileURLToPath(new URL('./skills/', import.meta.url))

export const name = 'creght-skills'
...
export function apply(ctx) {
  ...
  if (loadSkillsSync().length === 0) {
    throw new Error(`creght: no SKILL.md bundles found under ${SKILLS_ROOT}`)
  }
...
function parseSkill(raw, directory, skillFile) {
  ...
  if (parsed === undefined) {
    throw new Error(`creght: ${skillFile} is missing YAML frontmatter`)
  }
  const skillName = stringField(parsed.data, 'name')
  const description = stringField(parsed.data, 'description')
  if (skillName === undefined || description === undefined) {
    throw new Error(`creght: ${skillFile} frontmatter requires name and description`)
  }
  if (!SKILL_NAME.test(skillName)) {
    throw new Error(`creght: invalid skill name "${skillName}"`)
  }
  return {
    name: skillName,
    description,
...
function sortSkills(skills) {
  ...
  for (const skill of skills) {
    if (names.has(skill.name)) {
      throw new Error(`creght: duplicate skill name "${skill.name}"`)
    }
